  ...
  <div>
    <div class="mapContent">
      <el-button class="backBtn" @click="$router.go(-1)">返回</el-button>
        ...
        <input class="search-input" placeholder="请输入地址回车搜索..." v-model="addressOption"  id="suggestId" value="百度" @keyup.enter="searchAddress()">
        <div id="searchResultPanel" style="border:1px solid #C0C0C0;width:150px;height:auto; display:none;"></div>
      </div>
      <div class="l-addMarkerBtns" v-if="!addMarkerModal">
        <el-button-group>
          <el-button type="ghost" icon="el-icon-location" class="button" :class="{buttonCur: isMarker}" @click="addMarker(1)"></el-button>
          <el-button type="ghost" icon="el-icon-menu" class="button" :class="{buttonCur: isWeilan}" @click="addMarker(2)"></el-button>
          <el-button type="ghost" icon="el-icon-circle-plus" class="button" :class="" @click="addMarker(3)"></el-button>
        </el-button-group>
      </div>
      <div class="add_modal" v-if="addMarkerModal">
        <div class="content">
          <el-form ref="form" :model="formData" label-width="60">
            ...
            <el-row>
              ...
              <el-form-item label="名称" prop="name" class="l-form-input form-mini">
                <el-input ref='name' placeholder="名称" v-model="formData.name"></el-input>
              </el-form-item>
              </el-col>
              <el-col :span="12">
              <el-form-item label="类型" prop="labelState" class="l-form-input form-mini">
                <el-select ref='labelState' placeholder="类型" v-model="formData.labelState">
            ...
            <el-row>
              ...
              <el-form-item label="监控类型" prop="monitoringType" class="l-form-input form-mini">
                <el-select ref='monitoringType' placeholder="监控类型" v-model="formData.monitoringType">
              ...
              <el-form-item label="范围" prop="scope" class="l-form-input form-mini">
                <el-select ref='scope' placeholder="范围" v-model="formData.scope">
          ...
          <div style="float: right;">
            <el-button type="primary" size="small" :loading="actionLoading" @click="addMarkerInfo('form')">确定</el-button>
            <el-button type="ghost" size="small" @click="cancel" style="margin-left: 8px;">取消</el-button>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    async selectYs (type) {
      ...
      if (this.drawType * 1 === 1) {
        let iconSize = new BMap.Size(26, 48);
        let myIcon = new BMap.Icon(iconUrl, iconSize, {
          anchor: new BMap.Size(13, 48)
        });
        let markers = new BMap.Marker(this.newMarker, {icon: myIcon});
        ...
        if (this.newMarker && this.newMarker.lat && this.newMarker.lng) {
          let url = `http://api.map.baidu.com/geocoder/v2/?callback=renderReverse&location=${this.newMarker.lat},${this.newMarker.lng}&output=json&pois=1&ak=ehokpezgpQESNRi1ld0fQmRSgAoO6YAG`;
          $.ajax({
            url: url,
            dataType: 'jsonp',
            xhrFields: {
        ...
        let polygon = new BMap.Polygon(this.drawPoints, {strokeColor: this.formData.color, fillColor: this.formData.color, strokeWeight: 3, strokeOpacity: 0.8, fillOpacity: 0.7});
        ...
        let iconSize = new BMap.Size(20, 24);
        let iconWLUrl = '/static/images/weilanTips.png';
        let myIcon = new BMap.Icon(iconWLUrl, iconSize, {
          anchor: new BMap.Size(10, 24)
        });
        let markers = new BMap.Marker(mPoint, {icon: myIcon});
        this.map.addOverlay(markers);
        if (!x || !y) return;
        if (x && y) {
          let url = `http://api.map.baidu.com/geocoder/v2/?callback=renderReverse&location=${x},${y}&output=json&pois=1&ak=ehokpezgpQESNRi1ld0fQmRSgAoO6YAG`;
          $.ajax({
            url: url,
            dataType: 'jsonp',
            xhrFields: {
    ...
    showDraggingInfo (markers) {
      ...
      markers.addEventListener('dragend', function (e) {
        ...
        gc.getLocation(e.point, function (rs) {
          ...
          let url = `http://api.map.baidu.com/geocoder/v2/?callback=renderReverse&location=${e.point.lat},${e.point.lng}&output=json&pois=1&ak=ehokpezgpQESNRi1ld0fQmRSgAoO6YAG`;
          $.ajax({
            url: url,
            dataType: 'jsonp',
            xhrFields: {
    ...
    async init () {
      ...
      let ac = new BMap.Autocomplete({'input': 'suggestId', 'location': this.map}); // 建立一个自动完成的对象
      let _this = this;
...
//         _this.clearMap();    // 清除地图上所有覆盖物
      ...
      this.drawingManager.addEventListener('overlaycomplete', function (e) {
        if (_this.drawType === 1) {
          let marker = e.overlay.point;
          _this.clearMap();
          if (!marker.lng || !marker.lat) return;
          _this.newMarker = new BMap.Point(marker.lng, marker.lat);
          let iconSize = new BMap.Size(26, 48);
          let myIcon = new BMap.Icon('/static/images/bz01.png', iconSize, {
            anchor: new BMap.Size(13, 48)
          });
          let markers = new BMap.Marker(_this.newMarker, {icon: myIcon});
          _this.map.addOverlay(markers);
          _this.formData.point = marker.lng + ',' + marker.lat;
          if (marker.lat && marker.lng) {
            let url = `http://api.map.baidu.com/geocoder/v2/?callback=renderReverse&location=${marker.lat},${marker.lng}&output=json&pois=1&ak=ehokpezgpQESNRi1ld0fQmRSgAoO6YAG`;
            $.ajax({
              url: url,
              dataType: 'jsonp',
              xhrFields: {
        ...
        if (_this.drawType === 2) {
          ...
          _this.clearMap();
          let polygon = new BMap.Polygon(path, {strokeColor: '#A90329', fillColor: '#A90329', strokeWeight: 3, strokeOpacity: 0.8, fillOpacity: 0.7});  // 创建多边形
          ...
          let iconUrl = '/static/images/weilanTips.png';
          let iconSize = new BMap.Size(20, 24);
          let myIcon = new BMap.Icon(iconUrl, iconSize, {
            anchor: new BMap.Size(10, 24)
          });
          ...
          _this.map.addOverlay(markers);
          if (x && y) {
            let url = `http://api.map.baidu.com/geocoder/v2/?callback=renderReverse&location=${y},${x}&output=json&pois=1&ak=ehokpezgpQESNRi1ld0fQmRSgAoO6YAG`;
            $.ajax({
              url: url,
              dataType: 'jsonp',
              xhrFields: {
